...
시간초과 됨. 길이가 10000이라서 스택 ㅈㄴ 쌓인 듯.
     */
    int getResult(int[] nums) {
        int max = 0;
        for (int i = 0; i < nums.length; i++) {
            Queue<Integer> queue = null;
            int temp = dfs(nums, 0, i, queue);
    ...
    int dfs(int[] nums, int count, int index, Queue<Integer> queue) {
        ...
        if (!queue.contains(nums[index])) {
            queue.add(nums[index]);
        }


        for (int i = index; i < nums.length; i++) {
            dfs(nums, count + 1, index + 1, queue);
        }
        return queue.size();
    }

    public int solution(int[] nums) {
        int answer = getResult(nums);
        return answer;
    }
